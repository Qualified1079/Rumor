...
object TopologyBuilder {
    ...
    private fun buildLine(t: Topology.Line, p: SimParamRegistry, rng: Random, scope: CoroutineScope): BuildResult {
        val nodes = (0 until t.length).map { SimNode(it, scope) }
        val edges = (0 until t.length - 1).map { i ->
            SimTransport(nodes[i], nodes[i + 1], conditioner(p, rng))
        }
    ...
    private fun buildMesh(t: Topology.Mesh, p: SimParamRegistry, rng: Random, scope: CoroutineScope): BuildResult {
        val nodes = (0 until t.nodes).map { SimNode(it, scope) }
        ...
        for (node in nodes) {
            ...
            for (peer in candidates.shuffled(rng).take(t.connectionsPerNode)) {
                val e = SimTransport(node, peer, conditioner(p, rng))
    ...
    private fun buildTwoCluster(t: Topology.TwoCluster, p: SimParamRegistry, rng: Random, scope: CoroutineScope): BuildResult {
        ...
        val nodes = (0 until total).map { SimNode(it, scope) }
        ...
        fun fullyMesh(group: Set<Int>) {
            ...
            for (i in g.indices) for (j in i + 1 until g.size) {
                edges.add(SimTransport(nodes[g[i]], nodes[g[j]], conditioner(p, rng)))
            }
        }
        ...
        for (bIdx in bridgeIdx) {
            for (lIdx in leftIdx) {
                edges.add(SimTransport(nodes[bIdx], nodes[lIdx], conditioner(p, rng), tags = setOf("bridge", "left-bridge")))
            }
            for (rIdx in rightIdx) {
                edges.add(SimTransport(nodes[bIdx], nodes[rIdx], conditioner(p, rng), tags = setOf("bridge", "right-bridge")))
            }
        }
        return BuildResult(
            nodes, edges,
    ...
    private fun buildHubSpoke(t: Topology.HubSpoke, p: SimParamRegistry, rng: Random, scope: CoroutineScope): BuildResult {
        val nodes = (0..t.spokes).map { SimNode(it, scope) }
        val edges = (1..t.spokes).map { i ->
            SimTransport(nodes[0], nodes[i], conditioner(p, rng), tags = setOf("spoke"))
        }
        return BuildResult(
            nodes, edges,
    ...
    private fun buildCustom(t: Topology.Custom, p: SimParamRegistry, rng: Random, scope: CoroutineScope): BuildResult {
        val nodes = (0 until t.nodeCount).map { SimNode(it, scope) }
        val edges = t.edges.map { spec ->
            ...
            SimTransport(nodes[spec.from], nodes[spec.to], conditioner(p, rng), tags = spec.tags.toSet())
        }

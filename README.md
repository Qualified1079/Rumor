...
- **LoRa bridges — DM bridging and USB transport:** `MeshtasticBridge` and `MeshCoreBridge` ship working BLE codecs (Meshtastic via `MeshPacket.decoded`, MeshCore via v3 BLE-NUS opcodes) and bridge `TEXT_MESSAGE_APP` / channel broadcasts. BRIDGE_VOUCHED (O17) is in place so bridged content can now propagate beyond direct peers when the bridge opts in to vouching. Not yet implemented: USB serial/CDC transport, DM bridging via PKC (framework in place via the pluggable `DmEnvelope` registry), multi-channel selection UI, device picker for multi-radio setups, MeshCore v1/v2 firmware fallback.
- **Threat model and overclaims:** Rumor is not Tor and not Signal. It provides better privacy than centralized messengers (no server, no phone number, end-to-end encrypted) but weaker anonymity than Tor (a co-located observer with multiple radios can correlate traffic; userId is on the wire for signature verification). Do not use Rumor as a sole defence against a state-level adversary targeting you specifically.
- **Forward secrecy:** sender-side FS only today — ephemeral X25519 per DM compose. Receiver-side FS (rotating recipient prekeys, O38) is the largest crypto-hygiene gap and the next protocol-level priority once an emulator-equipped session is available.
- **UI for new features:** several new capabilities have core/protocol implementations but no UI yet — identity rotation (O41), self-presence beacons (O30/O57), transfer pause/cancel (O18), scheduled messages (O22), bridged-source labels (O47). Tracked in `CLAUDE.md`.
- **Backlog:** Living list of open items, design decisions, and completed gaps lives in `CLAUDE.md`. Notable open protocol work: receiver-side FS (O38), sealed-sender DM tags (O53), heal-storm token bucket (O43), plugin runtime loader (O15/O24/O26).

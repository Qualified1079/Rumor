...
| `rumor-room-posting-cert-v2:` (signable-bytes domain tag) | O89 posting cert sig scope | Set in `roomPostingCertSignableBytes`. Length-prefixed framing (O156). Bumping requires `-v3:` and forces every issued cert to be re-signed. (`-v1:` retired — see below.) |
...
| `rumor-keyword-filter-v1:` (signable-bytes domain tag) | 2026-07-31 (O156, pre-release hard cutover) | `rumor-keyword-filter-v2:` | v1 joined free-text `name` and each entry's free-text `pattern` with bare `\|`/`:`/`,` → an observer of one signed list could shift content across a field boundary and reuse the signature (splice forgery, same class as O144). v2 length-prefixes every field via `framed()`/`framedList()`. Hard cutover (no shipped users); v1 sigs never verify under v2. Reserved forever. |
| `rumor-message-delete-v1:` (signable-bytes domain tag) | 2026-07-31 (O156) | `rumor-message-delete-v2:` | v1 was `"...v1:$messageId\|$issuerPublicKey"`; messageId is sender-chosen (not hex-gated inbound) so the bare `\|` was re-partitionable. v2 length-prefixes both fields. Hard cutover. Reserved forever. |
| `rumor-bridge-vouched-v1:` (signable-bytes domain tag) | 2026-07-31 (O156) | `rumor-bridge-vouched-v2:` | v1 joined free-text `originNetwork`/`originSenderId`/`payload` (foreign-network data) with bare `\|`. v2 length-prefixes every field. Hard cutover. Reserved forever. |
| `rumor-room-create-v1:` (signable-bytes domain tag) | 2026-07-31 (O156/O157) | `rumor-room-create-v2:` | v1 joined free-text room `name` with bare `\|`. v2 length-prefixes every field. Latent (unwired until O79/O187) but fixed pre-emptively. Hard cutover. Reserved forever. |
| `rumor-room-action-v1:` (signable-bytes domain tag) | 2026-07-31 (O156/O157) | `rumor-room-action-v2:` | v1 joined free-text moderator `reason` with bare `\|`. v2 length-prefixes every field. Latent (unwired). Hard cutover. Reserved forever. |
| `rumor-room-posting-cert-v1:` (signable-bytes domain tag) | 2026-07-31 (O156/O157) | `rumor-room-posting-cert-v2:` | v1 joined free-text `channel` with bare `\|`. v2 length-prefixes every field. Latent (unwired). Hard cutover. Reserved forever. |

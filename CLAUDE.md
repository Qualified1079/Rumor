...
| O12 | **`deterministic-replay` residual non-determinism** | `SimTransport.exchange()` was fixed to (a) drop real `delay()` calls that defeated `speedMultiplier` and tripped the wall-clock cap, and (b) take the tick's seeded RNG so packet-loss decisions match across runs (commit 13a96be). What's still non-deterministic: `GossipEngine` runs internal handlers on `Dispatchers.Default`, so when `deliverExchange()` enqueues work, the coroutine scheduler order isn't guaranteed. The replay assertion only compares two coarse counters (`totalMessages`, `totalDropped`), so it's expected to pass anyway. If it still flakes after the SimTransport fix, the next step is making the engine's exchange handling synchronous-on-tick (either a single-threaded dispatcher for the sim's `SimNode.scope`, or having `deliverExchange()` block until ingest completes). Not done preemptively because it's a real architectural change with app-side implications and probably isn't needed. |

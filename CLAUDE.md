...
| O5 | **Bridges — DM bridging** | Both bridges deliberately broadcast-only today. Two viable architectures; **read the "Encrypted bridging" decision below before designing this**. Recommended split: Architecture B (envelope passthrough, true E2E) for Meshtastic since curves align; Architecture A (decrypt-and-re-encrypt) for MeshCore. Both require the bridged-contact UI labelling listed under Architecture A as hard pre-ship blockers. |
...
- **Encrypted bridging — two architectures, pick one before designing O5.**
  ...
  **Architecture B — envelope passthrough (bridge sees ciphertext only).** Rumor's DM envelope for `meshtastic:*` contacts becomes byte-compatible with Meshtastic's PKI envelope (X25519 + AES-CCM, Meshtastic framing). The bridge forwards opaque ciphertext both ways and never holds plaintext. Preserves real E2E across the bridge. Requires:
    1. A second DM-encryption code path in `GossipEngine.composeDirect` selected by recipient userId prefix (`meshtastic:*` → Meshtastic envelope; native → existing X25519+AES-GCM).
    2. Bridge advertises discovered Meshtastic NodeInfo as synthetic Rumor contacts whose `publicKey` field holds the Meshtastic X25519 pubkey (32 bytes, same curve).
    3. Rumor's Ed25519 signature is dropped for bridged DMs — Meshtastic's envelope has no separate signature field, only the AEAD tag. Acceptable because authenticity is enforced by the AEAD + the recipient's knowledge of which key sent it.
    4. This **does not work for MeshCore** with a shared design — MeshCore uses a different crypto stack. MeshCore bridging would need either Architecture A or its own per-bridge passthrough envelope.

  Default recommendation: **Architecture B for Meshtastic** (curves align cleanly, true E2E worth the per-bridge envelope code), **Architecture A for MeshCore** (no curve alignment to exploit). Both require the bridged-contact UI labelling from A1 above — Architecture B still has a bridge in the path, users still need to know that.

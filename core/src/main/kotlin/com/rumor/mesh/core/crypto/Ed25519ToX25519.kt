 ...
 * map.
 *
 * Ported from the O91 work on claude/practical-archimedes-wmySm
 * (commits c44087e + 31bd490) — this branch predates that branch's
 * KMP layout, so the file lives in the plain JVM source set here.
 * Same derivation libsodium exposes as
 * `crypto_sign_ed25519_{pk,sk}_to_curve25519`.
 *
 * Wired into the production DM path: `GossipEngine.composeDirect`
 * converts the recipient's Ed25519 pubkey before `x25519Agreement`,
 * and `ThreadViewModel.decryptPayload` converts the local Ed25519
 * seed (zeroed after use).
 *
 * **Verification:** `Ed25519DmRoundtripTest` asserts the property
 * that makes a DM decryptable — sender encrypts against the
 * converted recipient pubkey with an ephemeral X25519 key, and the
 * recipient's converted seed derives the identical shared secret.
 */
